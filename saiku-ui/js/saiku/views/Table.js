...
var Table = Backbone.View.extend({
        ...
        ignoreRightClick: true,
         build: function($trigger, e) {
            ...
            var citems = {
                    ...
                    "sep1": "---------",
                    "keeponly": {name: "Оставить только", payload: keep_payload }
            };
            if (d != "Measures") {
                citems["getchildren"] = {name: "Показать дочерние элементы", payload: children_payload }
                citems["fold1key"] = {
                        name: "Включая уровень",
                        items: lvlitems("include-")
                    };
                citems["fold2key"] = {
                        name: "Оставить и включая уровень",
                        items: lvlitems("keep-")
                    };
                citems["fold3key"] = {
                        name: "Убрать уровень",
            ...
            for (var col = 0; col < table[row].length; col++) {
                ...
                if (header.type === "COLUMN_HEADER" && header.value === "null" && (firstColumn == null || col < firstColumn)) {
                    ...
                    if (isLastColumn) {
                        ...
                        contents += '<th class="col" style="text-align: center;" colspan="' + colSpan + '" title="' + header.value + '"><div rel="' + row + ":" + col +'" class="i18n">' + header.value + '</div></th>';
                    } else {
                        // All the rest...
                        ...
                        if (header.value != nextHeader.value || isHeaderLowestLvl || groupChange || maxColspan) {
                            if (header.value == "null") {
                                ...
                                contents += '<th class="col" style="text-align: center;" colspan="' + (colSpan == 0 ? 1 : colSpan) + '" title="' + header.value + '"><div rel="' + row + ":" + col +'" class="i18n">' + header.value + '</div></th>';
                            }
                            colSpan = 1;
                        } else {
                            colSpan++;
                    ...
                    contents += '<th class="row_header"><div class="i18n">' + header.value + '</div></th>';
                    isHeaderLowestLvl = true;
    ...
    post_process: function() {
        if (this.workspace.query.get('type') == 'QM' && Settings.MODE != "view") {
            ...
            $(this.el).find('th.row div, th.col div').addClass('i18n');
        }
