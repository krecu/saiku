...
var WorkspaceToolbar = Backbone.View.extend({
    ...
    export_pdf: function(event) {
        window.location = 'http://'+ document.location.hostname +'/saiku/pdf?uri=' +
    ...
    email_pdf: function(event) {
        // Origin fix for IExplorer
        if (!window.location.origin) {
            window.location.origin = window.location.protocol + "//" + window.location.hostname + (window.location.port ? ':' + window.location.port: '');
        }            

        var fileLink = window.location.origin + Settings.REST_URL +
            Saiku.session.username + "/query/" + 
            this.workspace.query.id + "/export/pdf/" + this.workspace.query.formatter;

        window.location.href = 'mailto:?subject=Отчет из модуля свободно-задаваемых отчетов ЕСКИ&body=Вам направлен отчет, который был сформирован в модуле свободно-задаваемых отчетов ЕСКИ. Чтобы скачать отчет в формате «pdf», перейдите по ссылке: '+fileLink;
    },
